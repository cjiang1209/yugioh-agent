...
class TestLP:
    ...
    def test_opp_damage(self, gs: MUDGameState):
        gs.update(_ev(EventType.DAMAGE, player="P2", is_opponent=True, amount=2000, new_lp=6000))
    ...
    def test_opp_recover(self, gs: MUDGameState):
        gs.update(_ev(EventType.RECOVER, player="P2", is_opponent=True, amount=1000, new_lp=9000))
    ...
    def test_opp_pay_lp(self, gs: MUDGameState):
        gs.update(_ev(EventType.PAY_LP, player="P2", is_opponent=True, amount=800, new_lp=7200))
...
class TestDraw:
    ...
    def test_my_draw_card_adds_named_entry(self, gs: MUDGameState):
        ...
        gs.update(_ev(EventType.DRAW_CARD, player="you", card_name="Dark Magician"))
        gs.update(_ev(EventType.DRAW_CARD, player="you", card_name="Blue-Eyes White Dragon"))
...
class TestSummon:
    ...
    def test_flip_summon(self, gs: MUDGameState):
        gs.update(_ev(EventType.FLIP_SUMMON, player="Player1", card_name="Kuriboh", card_spec="m1"))
        assert len(gs.my_mzone) == 1
    ...
    def test_opp_set_spell_with_o_prefix(self, gs: MUDGameState):
        ...
        gs.update(
            _ev(EventType.SET, player="P2", is_opponent=True, card_spec="os1", position="face-down")
        )
    ...
    def test_summon_removes_from_hand(self, gs: MUDGameState):
        ...
        gs.my_hand.append(CardEntry(name="Dark Magician", code=46986414, spec="h1"))
        gs.update(
            _ev(
                EventType.SUMMON,
                player="P1",
    ...
    def test_set_removes_from_hand(self, gs: MUDGameState):
        ...
        gs.my_hand.append(CardEntry(name="Kuriboh", code=40640057, spec="h1"))
        gs.update(
            _ev(
                EventType.SET,
                player="you",
    ...
    def test_set_spell_removes_from_hand(self, gs: MUDGameState):
        ...
        gs.my_hand.append(CardEntry(name="Mirror Force", code=44095762, spec="h1"))
        gs.update(
            _ev(
                EventType.SET,
                player="you",
    ...
    def test_sp_summon_from_gy_doesnt_remove_hand(self, gs: MUDGameState):
        ...
        gs.my_graveyard.append(CardEntry(name="Dark Magician", code=46986414))
...
class TestMovement:
    def _summon(self, gs: MUDGameState, name: str, spec: str, opponent: bool = False) -> None:
        gs.update(
            _ev(
                EventType.SUMMON,
                player="P2" if opponent else "P1",
    ...
    def test_destroy(self, gs: MUDGameState):
        ...
        assert len(gs.my_mzone) == 1
        gs.update(_ev(EventType.DESTROY, card_spec="m1", card_name="Dark Magician"))
        assert len(gs.my_mzone) == 0

    def test_to_graveyard(self, gs: MUDGameState):
        self._summon(gs, "Dark Magician", "m1")
        gs.update(
            _ev(EventType.TO_GRAVEYARD, player="you", card_spec="m1", card_name="Dark Magician")
        )
        assert len(gs.my_mzone) == 0
    ...
    def test_banished(self, gs: MUDGameState):
        self._summon(gs, "Dark Magician", "m1")
        gs.update(_ev(EventType.BANISHED, player="you", card_spec="m1", card_name="Dark Magician"))
        assert len(gs.my_mzone) == 0
        assert len(gs.my_banished) == 1

    def test_to_hand(self, gs: MUDGameState):
        self._summon(gs, "Dark Magician", "m1")
        gs.update(_ev(EventType.TO_HAND, player="you", card_spec="m1", card_name="Dark Magician"))
        assert len(gs.my_mzone) == 0
    ...
    def test_to_deck(self, gs: MUDGameState):
        self._summon(gs, "Dark Magician", "m1")
        gs.update(_ev(EventType.TO_DECK, player="you", card_spec="m1", card_name="Dark Magician"))
        assert len(gs.my_mzone) == 0

    def test_tribute(self, gs: MUDGameState):
        self._summon(gs, "Kuriboh", "m1")
        gs.update(_ev(EventType.TRIBUTE, player="you", card_spec="m1", card_name="Kuriboh"))
        assert len(gs.my_mzone) == 0

    def test_discard_from_hand(self, gs: MUDGameState):
        gs.my_hand.append(CardEntry(name="Kuriboh", code=40640057, spec="h1"))
        gs.update(_ev(EventType.DISCARD, player="you", card_spec="h1", card_name="Kuriboh"))
        assert len(gs.my_hand) == 0
    ...
    def test_chaining_from_own_hand(self, gs: MUDGameState):
        ...
        gs.my_hand.append(CardEntry(name="Kuriboh", code=40640057, spec="h1"))
        gs.update(_ev(EventType.CHAINING, player="you", card_spec="h1", card_name="Kuriboh"))
        assert len(gs.my_hand) == 0
    ...
    def test_chaining_from_field_no_hand_removal(self, gs: MUDGameState):
        ...
        gs.my_hand.append(CardEntry(name="Mirror Force", code=44095762, spec="h1"))
        gs.update(_ev(EventType.CHAINING, player="you", card_spec="s1", card_name="Mirror Force"))
    ...
    def test_to_graveyard_from_own_hand(self, gs: MUDGameState):
        ...
        gs.my_hand.append(CardEntry(name="Kuriboh", code=40640057, spec="h1"))
        gs.update(_ev(EventType.TO_GRAVEYARD, player="you", card_spec="h1", card_name="Kuriboh"))
        assert len(gs.my_hand) == 0
        assert len(gs.my_graveyard) == 1
    ...
    def test_banished_from_own_hand(self, gs: MUDGameState):
        ...
        gs.my_hand.append(CardEntry(name="Kuriboh", code=40640057, spec="h1"))
        gs.update(_ev(EventType.BANISHED, player="you", card_spec="h1", card_name="Kuriboh"))
        assert len(gs.my_hand) == 0
        assert len(gs.my_banished) == 1
...
class TestNoLookup:
    def test_summon_without_lookup(self, gs_no_lookup: MUDGameState):
        gs_no_lookup.update(
            _ev(EventType.SUMMON, player="P1", card_name="Dark Magician", position="face-up attack")
        )
...
class TestChainingBareSpec:
    ...
    def test_opp_chain_field_spec_no_hand_decrement(self, gs: MUDGameState):
        ...
        gs.opp_hand_count = 3
        gs.opp_szone.append(CardEntry(name="Mirror Force", code=44095762, spec="os1"))
        gs.update(
            _ev(
                EventType.CHAINING,
                player="P2",
...
class TestTributeAddsToGY:
    def _summon(self, gs, name, spec, opponent=False):
        gs.update(
            _ev(
                EventType.SUMMON,
                player="P2" if opponent else "you",
                is_opponent=opponent,
                card_name=name,
                card_spec=spec,
                position="face-up attack",
...
class TestDestroyAddsToGY:
    ...
    def test_destroy_opp_adds_to_gy(self, gs: MUDGameState):
        ...
        gs.update(
            _ev(EventType.DESTROY, is_opponent=True, card_spec="m1", card_name="Dark Magician")
        )
        assert len(gs.opp_mzone) == 0
        assert len(gs.opp_graveyard) == 1
...
class TestMillSpecGuard:
    def test_mill_does_not_remove_existing_gy_card(self, gs: MUDGameState):
        ...
        gs.my_graveyard.append(CardEntry(name="Dark Magician", code=46986414))
        gs.update(
            _ev(EventType.TO_GRAVEYARD, player="you", card_spec="", card_name="Dark Magician")
        )
    ...
    def test_mill_opp_does_not_remove_existing(self, gs: MUDGameState):
        ...
        gs.opp_graveyard.append(CardEntry(name="Blue-Eyes White Dragon", code=89631139))
        gs.update(
            _ev(
                EventType.TO_GRAVEYARD,
                player="P2",
    ...
    def test_banish_from_deck_does_not_remove_existing(self, gs: MUDGameState):
        ...
        gs.my_banished.append(CardEntry(name="Dark Magician", code=46986414))
        gs.update(_ev(EventType.BANISHED, player="you", card_spec="", card_name="Dark Magician"))
...
class TestSpecPrefixNorm:
    ...
    def test_remove_from_field_no_prefix_finds_oprefix(self, gs: MUDGameState):
        ...
        gs.opp_mzone.append(CardEntry(name="Dark Magician", code=46986414, spec="m1"))
        removed = gs._remove_from_field("om1", "Dark Magician")
        assert removed is not None
        assert len(gs.opp_mzone) == 0

    def test_exact_match_still_works(self, gs: MUDGameState):
        gs.my_mzone.append(CardEntry(name="Dark Magician", code=46986414, spec="m1"))
...
class TestCrossPlayerDestroy:
    def test_destroy_opp_card_goes_to_opp_gy(self, gs: MUDGameState):
        ...
        gs.opp_szone.append(CardEntry(name="One for One", code=2295440, spec="os1"))
        gs.update(_ev(EventType.DESTROY, card_spec="os1", card_name="One for One"))
        assert len(gs.opp_szone) == 0
        assert len(gs.opp_graveyard) == 1
    ...
    def test_destroy_own_card_goes_to_own_gy(self, gs: MUDGameState):
        ...
        gs.my_szone.append(CardEntry(name="Mirror Force", code=44095762, spec="s1"))
        gs.update(_ev(EventType.DESTROY, card_spec="s1", card_name="Mirror Force"))
        assert len(gs.my_szone) == 0
        assert len(gs.my_graveyard) == 1
...
class TestSpellActivationFromHandToGY:
    def test_spell_from_hand_then_to_gy(self, gs: MUDGameState):
        ...
        gs.my_hand.append(CardEntry(name="Raigeki", code=12580477, spec="h1"))
        # CHAINING removes from hand
        gs.update(_ev(EventType.CHAINING, player="you", card_spec="h1", card_name="Raigeki"))
        assert len(gs.my_hand) == 0
        # TO_GRAVEYARD with field spec "s3" — card was never on szone
        gs.update(_ev(EventType.TO_GRAVEYARD, player="you", card_spec="s3", card_name="Raigeki"))
...
class TestExtraDeck:
    ...
    def test_to_extra_deck_own(self, gs: MUDGameState):
        ...
        gs.update(
            _ev(EventType.TO_EXTRA_DECK, player="you", card_spec="m1", card_name="Decode Talker")
        )
        assert len(gs.my_mzone) == 0
...
class TestFromGYBanished:
    def test_from_gy_to_field_own(self, gs: MUDGameState):
        gs.my_graveyard.append(CardEntry(name="Dark Magician", code=46986414, spec="g1"))
        gs.update(
            _ev(
                EventType.FROM_GY_TO_FIELD,
                player="you",
    ...
    def test_from_gy_to_field_opp(self, gs: MUDGameState):
        gs.opp_graveyard.append(CardEntry(name="Kuriboh", code=40640057, spec="og1"))
        gs.update(
            _ev(
                EventType.FROM_GY_TO_FIELD,
                player="P2",
    ...
    def test_from_gy_to_szone(self, gs: MUDGameState):
        gs.my_graveyard.append(CardEntry(name="Mirror Force", code=44095762, spec="g1"))
        gs.update(
            _ev(
                EventType.FROM_GY_TO_FIELD,
                player="you",
    ...
    def test_from_banished_to_field_own(self, gs: MUDGameState):
        gs.my_banished.append(CardEntry(name="Dark Magician", code=46986414, spec="r1"))
        gs.update(
            _ev(
    ...
    def test_from_banished_to_field_opp(self, gs: MUDGameState):
        gs.opp_banished.append(CardEntry(name="Kuriboh", code=40640057, spec="or1"))
        gs.update(
            _ev(
                EventType.FROM_BANISHED_TO_FIELD,
                player="P2",
...
class TestNonfieldRemoval:
    def test_gy_to_hand(self, gs: MUDGameState):
        gs.my_graveyard.append(CardEntry(name="Dark Magician", code=46986414, spec="g1"))
        gs.update(_ev(EventType.TO_HAND, player="you", card_spec="g1", card_name="Dark Magician"))
        assert len(gs.my_graveyard) == 0
        assert any(c.name == "Dark Magician" for c in gs.my_hand)

    def test_banished_to_gy(self, gs: MUDGameState):
        gs.my_banished.append(CardEntry(name="Kuriboh", code=40640057, spec="r1"))
        gs.update(_ev(EventType.TO_GRAVEYARD, player="you", card_spec="r1", card_name="Kuriboh"))
        assert len(gs.my_banished) == 0
        assert len(gs.my_graveyard) == 1
        assert gs.my_graveyard[0].name == "Kuriboh"
    ...
    def test_gy_to_banished(self, gs: MUDGameState):
        gs.my_graveyard.append(CardEntry(name="Dark Magician", code=46986414, spec="g1"))
        gs.update(_ev(EventType.BANISHED, player="you", card_spec="g1", card_name="Dark Magician"))
        assert len(gs.my_graveyard) == 0
        assert len(gs.my_banished) == 1

    def test_banished_to_deck(self, gs: MUDGameState):
        gs.my_banished.append(CardEntry(name="Dark Magician", code=46986414, spec="r1"))
        gs.update(_ev(EventType.TO_DECK, player="you", card_spec="r1", card_name="Dark Magician"))
        assert len(gs.my_banished) == 0

    def test_extra_to_gy(self, gs: MUDGameState):
        gs.my_extra.append(CardEntry(name="Decode Talker", spec="x1"))
        gs.update(
            _ev(EventType.TO_GRAVEYARD, player="you", card_spec="x1", card_name="Decode Talker")
        )
...
class TestControlChange:
    ...
    def test_opp_card_changes_to_us(self, gs: MUDGameState):
        ...
        gs.opp_mzone.append(CardEntry(name="Blue-Eyes White Dragon", code=89631139, spec="om1"))
        gs.update(
            _ev(
                EventType.CONTROL_CHANGE,
                player="P2",
...
class TestSwap:
    def test_swap_moves_card(self, gs: MUDGameState):
        gs.my_mzone.append(CardEntry(name="Dark Magician", code=46986414, spec="m1"))
        gs.update(_ev(EventType.SWAP, card_name="Dark Magician", target_spec="om2"))
        assert len(gs.my_mzone) == 0
        assert len(gs.opp_mzone) == 1
        assert gs.opp_mzone[0].spec == "om2"
...
class TestParseCardLine:
    ...
    def test_with_level(self):
        name, pos = _parse_card_line("Dark Magician face-up attack level 7")
        assert name == "Dark Magician"
        assert pos == "face-up attack"

    def test_tricky_name(self):
        name, pos = _parse_card_line("Attack Gainer face-up attack level 4")
    ...
    def test_link_rating(self):
        name, pos = _parse_card_line("Decode Talker face-up attack link rating 3")
        assert name == "Decode Talker"
        assert pos == "face-up attack"

    def test_rank(self):
        name, pos = _parse_card_line("Number 39: Utopia face-up attack rank 4")
    ...
    def test_level_warrior(self):
        ...
        name, pos = _parse_card_line("Level Warrior face-up attack level 3")
...
class TestDeckCount:
    ...
    def test_mill_to_gy_decrements_deck(self, gs: MUDGameState):
        ...
        gs.update(
            _ev(EventType.TO_GRAVEYARD, player="you", card_name="Dark Magician", card_spec="")
        )
    ...
    def test_search_to_hand_decrements_deck(self, gs: MUDGameState):
        ...
        gs.my_deck_count = 35
        # Empty spec = no recognized zone prefix → assumed deck source
        gs.update(_ev(EventType.TO_HAND, player="you", card_name="Dark Magician", card_spec=""))
        assert gs.my_deck_count == 34
        assert len(gs.my_hand) == 1
    ...
    def test_banish_from_deck_decrements(self, gs: MUDGameState):
        ...
        gs.my_deck_count = 35
        # Empty spec = no recognized zone prefix → assumed deck source
        gs.update(_ev(EventType.BANISHED, player="you", card_name="Dark Magician", card_spec=""))
        assert gs.my_deck_count == 34
    ...
    def test_to_deck_increments(self, gs: MUDGameState):
        ...
        gs.update(_ev(EventType.TO_DECK, player="you", card_name="Kuriboh", card_spec="g1"))
    ...
    def test_to_deck_unknown_source_increments(self, gs: MUDGameState):
        ...
        gs.update(_ev(EventType.TO_DECK, player="you", card_name="Unknown Card", card_spec=""))
        assert gs.my_deck_count == 31
...
class TestEventSequence:
    def test_basic_duel_flow(self, gs: MUDGameState):
        ...
        gs.update(_ev(EventType.DRAW, player="you", amount=1))
        gs.update(_ev(EventType.DRAW_CARD, player="you", card_name="Mystical Space Typhoon"))
        gs.update(_ev(EventType.NEW_PHASE, phase="main1 phase"))
        ...
        gs.update(_ev(EventType.NEW_PHASE, phase="battle phase"))
        gs.update(_ev(EventType.ATTACK, player="P1", card_spec="m1", card_name="Dark Magician"))
        gs.update(_ev(EventType.DAMAGE, player="P2", is_opponent=True, amount=2500, new_lp=5500))
        ...
        gs.update(_ev(EventType.NEW_PHASE, phase="draw phase"))
        gs.update(_ev(EventType.DRAW, player="P2", is_opponent=True, amount=1))
...
class TestClearSpecOnGYBanished:
    ...
    def test_banished_clears_spec(self, gs: MUDGameState):
        ...
        gs.update(_ev(EventType.BANISHED, player="you", card_spec="m2", card_name="Kuriboh"))
...
class TestPosChangeSzone:
    def test_pos_change_szone(self, gs: MUDGameState):
        ...
        gs.update(
            _ev(EventType.POS_CHANGE, card_spec="s1", card_name="Mirror Force", position="face-up")
        )
        assert gs.my_szone[0].position == "face-up"

    def test_pos_change_opp_szone(self, gs: MUDGameState):
        gs.opp_szone.append(CardEntry(name="", spec="os1", position="face-down"))
        gs.update(
            _ev(EventType.POS_CHANGE, card_spec="os1", card_name="Trap Card", position="face-up")
        )
...
class TestSpSummonExtraDeck:
    ...
    def test_sp_summon_does_not_remove_if_in_gy(self, gs: MUDGameState):
        ...
        gs.my_extra.append(CardEntry(name="Dark Magician", code=46986414, spec="x1"))
        gs.my_graveyard.append(CardEntry(name="Dark Magician", code=46986414, spec=""))
        gs.update(
            _ev(
                EventType.SP_SUMMON,
                player="Player1",
    ...
    def test_opp_sp_summon_revival_no_hand_decrement(self, gs: MUDGameState):
        ...
        gs.opp_hand_count = 3
        gs.opp_graveyard.append(CardEntry(name="Dark Magician", code=46986414))
        gs.update(
            _ev(
                EventType.SP_SUMMON,
                player="P2",
...
class TestXYZMaterial:
    def test_xyz_attach_removes_from_field(self, gs: MUDGameState):
        gs.my_mzone.append(CardEntry(name="Kuriboh", code=40640057, spec="m1"))
        gs.update(
            _ev(
    ...
    def test_xyz_detach_is_noop(self, gs: MUDGameState):
        ...
        gs.update(_ev(EventType.XYZ_DETACH, player="you", card_name="Kuriboh"))
        assert len(gs.my_graveyard) == 0
